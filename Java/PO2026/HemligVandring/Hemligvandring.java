...
public class Hemligvandring {
    ...
    public static void main(String[] args) {
        ...
        if (asks[0] == -1) {
            ...
            for (int i = 0; i < n; i++) {
                city.block(n / 2 - (n / 2 % 2), i);
            }
            int t = ask();
            if (t == -1) {
                suspect = new ArrayList<>(block);
            } else if (t != d) {
                suspect = new ArrayList<>(suspect.subList(si / 2, 3 * si / 4));
            } else {
                suspect = new ArrayList<>(suspect.subList(3 * si / 4, si));
            }
            si = suspect.size();
            while (si != 1) {
                block = new ArrayList<>(suspect.subList(0, si / 2));
                for (int i = 0; i < block.size(); i++) {
                    int[] su = suspect.get(i);
                    city.block(su[0] - 1, su[1]);
                    city.block(su[0], su[1] - 1);
                ...
                t = ask();
                suspect = ((t == -1) ? new ArrayList<>(block) : new ArrayList<>(suspect.subList(si / 2, si)));
                si = suspect.size();
            }
            ...
            si = suspect.size();
            while (si > 1) {
                block = new ArrayList<>(suspect.subList(0, si / 3));
                for (int i = 0; i < block.size(); i++) {
                    int[] su = suspect.get(i);
                    city.block(su[0] - 1, su[1]);
                    city.block(su[0], su[1] - 1);
                ...
                ArrayList<int[]> hinder = new ArrayList<>(suspect.subList(si / 3, 2 * si / 3));
                for (int i = 0; i < hinder.size(); i++) {
                    int[] su = suspect.get(i + si / 3);
                    if (hus[0] <= su[0]) {
                        city.block(su[0] - 1, su[1]);
                    }else{
                        city.block(su[0] +1, su[1]);
                ...
                if (t == -1) {
                    ...
                    suspect = new ArrayList<>(hinder);
                } else {
                    suspect = new ArrayList<>(suspect.subList(2 * si / 3, si));
                }
                si = suspect.size();
            }
